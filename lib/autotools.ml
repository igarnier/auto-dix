...
module Pair(O : Map.OrderedType) =
struct

  type t = O.t * O.t

  let compare (a,b) (c,d) =
    match O.compare a c with
    | 0 -> O.compare b d
    | c -> c

end

module UnorderedPair(O : Map.OrderedType) =
struct

  type t = O.t * O.t

  let compare (a,b as x) (c,d as y) =
    let (a, b) =
      let v = O.compare a b in
      if v <= 0 then
        x
      else
        (b, a)
    and (c, d) =
      let v = O.compare c d in
      if v <= 0 then
        y
      else
        (d, c)
    in
    let v = O.compare a c in
    if v = 0 then
      O.compare b d
    else
      v
...
let cartesian_product l1 l2 =
  List.fold_left (fun prod elt1 ->
      List.fold_left (fun prod elt2 ->
          (elt1, elt2) :: prod
        ) prod l2
    ) [] l1
...
(* Multiset *)
module Multiset(O : Map.OrderedType) =
struct
...
struct
  ...
  let realloc ({ fill; arr; default } as a) =
    ...
    let arr'     = Array.make new_size default in
...
struct
  ...
  (* This module contains code from the ExtLib library under the GPL license,
